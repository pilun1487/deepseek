-- ==============================================
-- МИНИМАЛЬНЫЙ FAKE TRADE + SPAWNER
-- Только фейк-трейды и спавн питомцев
-- ==============================================
...
-- ==============================================
-- 1. ХУКИ ДЛЯ ФЕЙК-ПЛЕЕРОВ (обязательно)
-- ==============================================
local fakePlayerIds = {}
...
-- Список высокоценных питомцев (Balloon Unicorn+)
local highValuePets = {
...
local function generateRandomPetProperties()
    local t = {'FR','NFR'}[math.random(1,2)]
    local props = { F = false, R = false, N = false }
    if t == 'FR' then props.F = true; props.R = true
    elseif t == 'NFR' then props.F = true; props.R = true; props.N = true end
    return props
end
...
for _, name in ipairs(funcs) do if TradeApp[name] then originalFuncs[name] = TradeApp[name] end end

TradeApp._get_local_trade_state = function(self)
    return mockState.active and TableUtil.deep_copy(mockState.trade) or originalFuncs._get_local_trade_state(self)
end
...
TradeApp._overwrite_local_trade_state = function(self, newState)
    if mockState.active then
        if newState then
            ...
            mockState.trade = nil; mockState.active = false; mockState.scamWarningShown = false; self.local_trade_state = nil
        end
    else
        return originalFuncs._overwrite_local_trade_state(self, newState)
    end
end
...
TradeApp._get_my_offer = function(self)
    ...
    if mockState.active and state then
        if Players.LocalPlayer == state.sender then return state.sender_offer, 'sender_offer'
        else return state.recipient_offer, 'recipient_offer' end
    end
    return originalFuncs._get_my_offer(self)
end

TradeApp._get_partner_offer = function(self)
    local state = self:_get_local_trade_state()
    if mockState.active and state then
        if Players.LocalPlayer == state.sender then return state.recipient_offer, 'recipient_offer'
        else return state.sender_offer, 'sender_offer' end
    end
    return originalFuncs._get_partner_offer(self)
end

TradeApp._get_my_player = function(self)
    return mockState.active and Players.LocalPlayer or originalFuncs._get_my_player(self)
end

TradeApp._get_partner = function(self)
    return mockState.active and mockState.trade.recipient or originalFuncs._get_partner(self)
end

TradeApp._get_current_trade_stage = function(self)
    return mockState.active and mockState.trade.current_stage or originalFuncs._get_current_trade_stage(self)
end
    ...
    if mockState.active then
        local function merge(a, b)
            for k,v in pairs(b) do
                if type(v)=='table' and a[k] and type(a[k])=='table' then merge(a[k], v) else a[k]=v end
            end
            return a
        end
        self:_overwrite_local_trade_state(merge(self:_get_local_trade_state(), changes))
    else
        return originalFuncs._change_local_trade_state(self, changes)
    end
end

TradeApp._get_lock_time = function(self)
    if mockState.active then
        if self:_get_current_trade_stage() == 'negotiation' then return CONFIG.NEGOTIATION_LOCK end
    ...
    return originalFuncs._get_lock_time(self)
end
...
TradeApp._lock_trade_for_appropriate_time = function(self)
    if mockState.active then
        ...
        return originalFuncs._lock_trade_for_appropriate_time(self)
    end
end
...
TradeApp._add_item_to_my_offer = function(self)
    if mockState.active and mockState.trade then
        ...
        if picked then
            local already = false
            for _, it in ipairs(mockState.trade.sender_offer.items) do if it.unique == picked.unique then already = true; break end end
            if not already then
                ...
                if BackpackApp.set_item_unique_hidden then BackpackApp:set_item_unique_hidden(picked.unique, 'TradeApp') end
            end
        end
        mockState.isAddingItem = false
    else
        return originalFuncs._add_item_to_my_offer(self)
    end
end
...
TradeApp._remove_item_from_my_offer = function(self, item)
    if mockState.active and mockState.trade then
        ...
        return originalFuncs._remove_item_from_my_offer(self, item)
    end
end
...
TradeApp._on_accept_pressed = function(self)
    if mockState.active and mockState.trade then
        ...
        return originalFuncs._on_accept_pressed(self)
    end
end
...
TradeApp._on_confirm_pressed = function(self)
    if mockState.active and mockState.trade then
        ...
        return originalFuncs._on_confirm_pressed(self)
    end
end
...
TradeApp._on_unaccept_pressed = function(self)
    if mockState.active and mockState.trade then
        ...
        return originalFuncs._on_unaccept_pressed(self)
    end
end
...
TradeApp._decline_trade = function(self, silent)
    if mockState.active then
        ...
        mockState.partnerActionPending = false; mockState.tradeCompleting = false; mockState.scamWarningShown = false
        self:_overwrite_local_trade_state(nil)
        UIManager.set_app_visibility('TradeApp', false)
        if BackpackApp.reset_hidden_item_tag then BackpackApp:reset_hidden_item_tag('TradeApp') end
    else
        return originalFuncs._decline_trade(self, silent)
    end
end
...
TradeApp._evaluate_trade_fairness = function(self)
    if mockState.active and mockState.trade and not mockState.scamWarningShown then
        ...
        return originalFuncs._evaluate_trade_fairness(self)
    end
end
...
function CreateFakePlayerCharacterFromPARTNER_NAME(partner_name, partner_id, petData, petFlags)
    ...
    if petData then
        ...
        if model then
            ...
            if petFlags and petFlags.M then applyMegaNeonEffects(model, kind)
            elseif petFlags and petFlags.N then applyNeonEffects(model, kind) end
            model.Parent = folder
            model:SetPrimaryPartCFrame(char.HumanoidRootPart.CFrame)
            model:ScaleTo(2)
            for _, p in ipairs(model:GetDescendants()) do if p:IsA('BasePart') then p:SetAttribute('IsFakePet', true) end end
            ...
            for _, p in pairs(char:GetDescendants()) do if p:IsA('BasePart') and not p.Massless then p.Massless = true; p:SetAttribute('HaveMass', true) end end
            local wrapper = {
                char = model, mega_neon = petFlags and petFlags.M or false, neon = petFlags and petFlags.N or false,
                player = { Name = partner_name, UserId = partner_id, Character = char },
                pet_unique = HttpService:GenerateGUID(false),
                pet_id = kind,
                location = { full_destination_id = 'housing', destination_id = 'housing', house_owner = { Name = partner_name, UserId = partner_id } },
            ...
            updateData('pet_char_wrappers', function(wrappers)
                wrapper.unique = #wrappers + 1; wrapper.index = #wrappers + 1
...
regB.MouseButton1Click:Connect(function() currentFakeType='regular'; regB.BackgroundColor3=Color3.fromRGB(60,160,60); neonB.BackgroundColor3=Color3.fromRGB(40,40,50); megaB.BackgroundColor3=Color3.fromRGB(40,40,50) end)
neonB.MouseButton1Click:Connect(function() currentFakeType='neon'; regB.BackgroundColor3=Color3.fromRGB(40,40,50); neonB.BackgroundColor3=Color3.fromRGB(60,160,60); megaB.BackgroundColor3=Color3.fromRGB(40,40,50) end)
megaB.MouseButton1Click:Connect(function() currentFakeType='mega'; regB.BackgroundColor3=Color3.fromRGB(40,40,50); neonB.BackgroundColor3=Color3.fromRGB(40,40,50); megaB.BackgroundColor3=Color3.fromRGB(60,160,60) end)
...
makeButton('Spawn Fake Player', Color3.fromRGB(80,60,180), function()
    ...
    local success, id = pcall(function() return Players:GetUserIdFromNameAsync(partnerBox.Text) end)
...
makeButton('Spawn High Tier', Color3.fromRGB(200,0,200), function()
    for _, name in ipairs(highValuePets) do
        ...
        if kind then
            ...
            local unique = HttpService:GenerateGUID(false)
            local inv = ClientData.get('inventory')
            inv.pets[unique] = { unique = unique, category = 'pets', id = kind, kind = kind, newness_order = math.random(1,900000), properties = props }
        end
    end
...
UserInputService.InputEnded:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 or input.UserInputType == Enum.UserInputType.Touch then dragging = false
end)
